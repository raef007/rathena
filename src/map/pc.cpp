...
void pc_makesavestatus(map_session_data *sd) {
	...
	// Persist auto-battle time to DB during autosave (do NOT stop the timer;
	// autobattle_stop is called separately on real logout in map_quit).
...
int32 pc_modifysellvalue( const map_session_data* sd, int32 orig_value )
{
	...
	if((skill=pc_checkskill(sd,MC_OVERCHARGE))>0)	// Merchant Overcharge
...
int32 pc_useitem(map_session_data *sd,int32 n)
{
	...
	/* Items with delayed consume are not meant to work while in mounts except reins of mount(12622) and Beast Master's Leash(50011) */
	if( id->flag.delay_consume > 0 ) {
		if( nameid != ITEMID_REINS_OF_MOUNT && nameid != 50011 && sd->sc.getSCE(SC_ALL_RIDING) )
			return 0;
		else if( pc_issit(sd) )
			return 0;
	}
